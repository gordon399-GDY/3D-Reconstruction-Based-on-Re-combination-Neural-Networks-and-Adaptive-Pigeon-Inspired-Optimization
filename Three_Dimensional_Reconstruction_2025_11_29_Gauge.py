...
class BPNN:
    def __init__(self, num_in, num_hidden, num_out):
        ...
        self.wight_in = [
            ...
            #
            [5.437694944452474, -7.806223283283583, 3.4010231484516193, 0.034119074785716776, 1.2228116168128274,
             -0.30933986987046547, 4.904352121016822, -7.414289798059994, 3.1015855611738656, -0.04710478970670941,
             0.5735930261628404, 0.3596677834410535],
            [0.12740669054821877, -5.014850113538859, -0.8477613831748844, 1.0170493195084434, -3.9092428052281813,
             -0.8337627663998691, 0.11600005201293703, -4.383986083039213, -0.878548675564849, 0.5426955763115496,
             -3.0077358823464193, -1.008895067560161],
            [- 0.0911658657340839, 4.287049084075241, -26.025559495688388, 15.61574137245841, 9.910987445701943,
             6.332991843877077, -0.350967249617043, 4.693163452197321, -25.57072835210723, 15.64010607988969,
             9.09444403376687, 6.504187149219303],
            [-0.3329881770198686, 0.5570343535578456, -1.891982037571118, -0.4828169874986403, -0.9438029484839108,
             -0.26154659350860215, -0.3312100312908608, 0.5586069476235483, -1.8900185930455378, -0.4787734956038044,
             -0.9491890356647626, -0.2583882796923472]
        ]
        self.wight_out = [
            # 以下为左摄像机的
    ...
    def plot_errors(self):
        ...
        plt.xticks(range(1, len(indices) + 1, 10))  # 每10个矢量显示一个刻度
        ...
        # plt.text(0.02, 0.98, f'最小指标: {min_error:.4f}\n最大指标: {max_error:.4f}\n平均指标: {avg_error:.4f}',
        ...
        for k, j in enumerate(pattern):
            for i in range(itera):  # 9500000
                ...
                if (i + 1) % 300000 == 0:
                    ...
                    # 存储每个矢量的最终误差（乘以10000后）
                    if (i + 1) == 300000:  # 只存储每个矢量的最终误差
...
def demo():
    ...
    patt = [
        # ... 您的147个矢量数据保持不变 ...
        [gdy3, [0.099833446, 0.250191289,  0.057222740, 0.200173460]],    #1
        [gdy3, [0.104689968, 0.260899279,  0.060715508, 0.208913893]],   #2
        [gdy3, [0.107408687, 0.248480474,  0.063392217, 0.199129123]],   #3
        [gdy3, [0.121604479, 0.246696995,  0.074487813, 0.197747390]],   #4
        [gdy3, [0.125922213, 0.257150376,  0.077570100, 0.206295089]],   #5
        [gdy3, [0.129027158, 0.245186119,  0.080547660, 0.196843464]],   #6
        [gdy3, [0.142826308, 0.244225459,  0.091386789, 0.196000664]],   #7
        [gdy3, [0.151040574, 0.253907323,  0.097668221, 0.204117134]],   #8
        [gdy3, [0.157464120, 0.241685203,  0.103360220, 0.194550521]],#  #9
      ]
